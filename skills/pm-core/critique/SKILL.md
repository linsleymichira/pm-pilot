...
If the user invokes the skill but no document text, file, or workspace reference is provided, ask the user to paste or attach the document to review.

## Process

### Step 1: Understand Intent (internal scratchpad)

Before critiquing, think silently in an internal `<scratchpad>` to evaluate and identify:
...
Do not output the scratchpad contents verbatim. Integrate the conclusions into the structured critique template so the final output remains the prescribed Markdown format. Calibrate critique intensity to stage and stakes. An early one-pager gets lighter scrutiny than a Full PRD about to be shipped.
...
If the provided text is too brief to evaluate against the six dimensions (for example, a one-sentence fragment or a short idea note), do NOT use the full critique template. Instead, ask the user to provide more context or the rest of the draft, and optionally suggest the specific missing information needed to run a full critique.
...
After the critique, append a **Recommended AI Skills** section. Select only the 1–2 most relevant tools based on your findings (do not list all tools every time). Examples:

- Recommend `synthesize-interviews` ONLY if the document relies on unvalidated user assumptions or needs user research.
- Recommend `prd` when the document requires structural changes or a rewrite into a PRD format.
- Recommend `market-research` if competitive claims lack evidence or the market context is unclear.
- Recommend `lenny-podcast` when the author would benefit from frameworks or product-management patterns for strategy.

Keep recommendations concise and justified by one short rationale sentence each.
...
- If the user invokes the skill but no document text, file, or workspace reference is found, politely ask them to provide or attach the document they want reviewed.

## Short / Fragment Inputs

- If the provided input is extremely short or fragmented (e.g., a single-sentence idea), do not run the full 6-dimension critique. Instead, ask for the complete draft or propose the specific additional context needed (audience, success metrics, scope, assumptions).
